...
class PushMetricsConfig:
    table_name: str = "push_metrics"
    schema: str = "mobile_app"
    date_column: str = "date"
    country_column: str = "country_id"
    procedure_call: str = "call an.prc_mobile_app_push_metrics(%s);"
...
config = PushMetricsConfig()
TIME_CONDITION = check_time_condition(time_to_check=config.time_check, comparison='>=')
MOBILE_APP_PUSH_GITLAB_DDL_Q, MOBILE_APP_PUSH_GITLAB_DDL_URL = fetch_gitlab_data(
    config_key="repstat",
    dir_name=config.proc_name_parsed,
    file_name=config.proc_name_parsed,
)
MOBILE_APP_PUSH_GITLAB_SELECT_Q, MOBILE_APP_PUSH_GITLAB_SELECT_Q_URL = fetch_gitlab_data(
    config_key="repstat",
    dir_name=config.proc_name_parsed,
...
def push_metrics_get_sql_instance(context, mobile_app_mau_result=None):
    """
    Loop over prod sql instances and create output dictionary with data to start on separate instance.

    Args:
        context (_type_): logs
        mobile_app_mau_result(list): pass result from previous step.
    ...
    if TIME_CONDITION:
        ...
        for sql_instance in Operations.generate_sql_instance(
                ...
                ddl_query=MOBILE_APP_PUSH_GITLAB_DDL_Q,
                select_query=MOBILE_APP_PUSH_GITLAB_SELECT_Q,):
            yield sql_instance
    else:
...
def push_metrics_query_on_db(context, sql_instance_country_query: dict):
    """
...
@op
def push_metrics_refresh_tableau_object(context, results):
    """
    Refresh tableau object
    """
    if TIME_CONDITION:
        datasource_id = tableau_object_uid['datasource_by_name']['mobile_app_push_metrics']
