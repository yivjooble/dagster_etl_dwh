...
from utility_hub.core_tools import fetch_gitlab_data, get_datediff, run_tableau_object_refresh
...
class ClickDataConfig:
    table_name: str = "click_data_agg"
    schema: str = "aggregation"
    date_column: str = "load_datediff"
    country_column: str = "country_id"
    procedure_call: str = "call an.prc_click_data_agg(%s);"
...
config = ClickDataConfig()

CLICK_DATA_GITLAB_DDL_Q, CLICK_DATA_GITLAB_DDL_URL = fetch_gitlab_data(
    config_key="repstat",
    dir_name=config.proc_name_parsed,
    file_name=config.proc_name_parsed,
)
CLICK_DATA_GITLAB_SELECT_Q, CLICK_DATA_GITLAB_SELECT_Q_URL = fetch_gitlab_data(
    config_key="repstat",
    dir_name=config.proc_name_parsed,
...
def click_data_agg_get_sql_instance(context, click_metric_result=None):
    '''
    ...
    for sql_instance in Operations.generate_sql_instance(
            ...
            ddl_query=CLICK_DATA_GITLAB_DDL_Q,
            select_query=CLICK_DATA_GITLAB_SELECT_Q, ):
        yield sql_instance


@op(retry_policy=retry_policy, required_resource_keys={'globals'})
def click_data_agg_query_on_db(context, sql_instance_country_query: dict):
    """
...
def click_data_agg_check_data_discrepancy(context, click_data_agg_result):
    """
    Execute data discrepancy review code on destination db.
    """
    return unified_check_data_discrepancy(
        context=context,
        procedure_name="click_data_agg"
    )


@op
def click_data_refresh_tableau_object(context, click_data_agg_result):
    """
    Refresh Tableau datasource: aggregation.click_data_agg.
    """
    datasource_id = tableau_object_uid['datasource_by_name']['aggregation_v_click_data_agg']
