...
class ActivityMetricsMauConfig:
    table_name: str = "activity_metrics"
    schema: str = "mobile_app"
    date_column: str = "date"
    country_column: str = "country_id"
    procedure_call: str = "call an.prc_mobile_app_activity_metrics_mau(%s, %s);"
    ...
    @property
    def get_month_date_range(self) -> tuple[str, str]:
...
config = ActivityMetricsMauConfig()
MONTH_DATE_START, MONTH_DATE_END = config.get_month_date_range
TIME_CONDITION = check_time_condition(time_to_check=config.time_check, comparison='>=')
MOBILE_APP_MAU_GITLAB_DDL_Q, MOBILE_APP_MAU_GITLAB_DDL_URL = fetch_gitlab_data(
    config_key="repstat",
    dir_name=config.proc_name_parsed,
    file_name=config.proc_name_parsed,
)
MOBILE_APP_MAU_GITLAB_SELECT_Q, MOBILE_APP_MAU_GITLAB_SELECT_Q_URL = fetch_gitlab_data(
    config_key="repstat",
    dir_name=config.proc_name_parsed,
...
def activity_metrics_mau_get_sql_instance(context, mobile_app_wau_result=None):
    """
    Loop over prod sql instances and create output dictionary with data to start on separate instance.

    Args:
        context (_type_): logs
        mobile_app_wau_result(list): pass result from previous step.
    ...
    if TIME_CONDITION:
        ...
        context.log.info(f'Selected countries: {launch_countries}\n'
                         f'Start procedures for: {MONTH_DATE_START} - {MONTH_DATE_END}\n'
        ...
        for sql_instance in Operations.generate_sql_instance(
                ...
                ddl_query=MOBILE_APP_MAU_GITLAB_DDL_Q,
                select_query=MOBILE_APP_MAU_GITLAB_SELECT_Q,):
            yield sql_instance
    else:
...
def activity_metrics_mau_query_on_db(context, sql_instance_country_query: dict):
    ...
    operation_date_start = MONTH_DATE_START
    operation_date_end = MONTH_DATE_END
