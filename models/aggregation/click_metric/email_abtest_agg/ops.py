...
class EmailAbTestConfig:
    table_name: str = "email_abtest_agg"
    schema: str = "aggregation"
    date_column: str = "load_date_diff"
    country_column: str = "country_id"
    procedure_call: str = "call an.prc_email_abtest_agg(%s);"
...
config = EmailAbTestConfig()

EMAIL_ABTEST_GITLAB_DDL_Q, EMAIL_ABTEST_GITLAB_DDL_URL = fetch_gitlab_data(
    config_key="repstat",
    dir_name=config.proc_name_parsed,
    file_name=config.proc_name_parsed,
)
EMAIL_ABTEST_GITLAB_SELECT_Q, EMAIL_ABTEST_GITLAB_SELECT_Q_URL = fetch_gitlab_data(
    config_key="repstat",
    dir_name=config.proc_name_parsed,
...
def email_abtest_agg_get_sqlinstance(context, click_metric_result=None):
    '''
    ...
    for sql_instance in Operations.generate_sql_instance(
            ...
            ddl_query=EMAIL_ABTEST_GITLAB_DDL_Q,
            select_query=EMAIL_ABTEST_GITLAB_SELECT_Q,):
        yield sql_instance


@op(retry_policy=retry_policy, required_resource_keys={'globals'})
def email_abtest_agg_query_on_db(context, sql_instance_country_query: dict):
...
def email_abtest_agg_check_data_discrepancy(context, email_abtest_result):
    """
    Execute data discrepancy review code on destination db.
    """
    return unified_check_data_discrepancy(
        context=context,
        result=email_abtest_result,
        procedure_name="email_abtest_agg"
    )
