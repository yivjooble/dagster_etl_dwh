...
TABLE_NAME = "sync_duplicate_events_marking_with_mssql"
SCHEMA = "public"
...
@op(out=Out(str))
def sync_duplicate_get_sql_query(context) -> str:
...
def sync_duplicate_get_sqlinstance(context, query):
    '''
...
def sync_duplicate_launch_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def sync_duplicate_events_marking_with_mssql_job():
    # start procedure on replica
    call_procedure_query = sync_duplicate_get_sql_query()
    replica_instances = sync_duplicate_get_sqlinstance(call_procedure_query)
    replica_instances.map(sync_duplicate_launch_query_on_db)
