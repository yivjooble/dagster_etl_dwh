...
TABLE_NAME = "dwh_postgres_exec_sql_code"


@op(required_resource_keys={"globals"})
def dwh_postgres_exec_sql_code_op(context):
...
@job(
    ...
    resource_defs={"globals": make_values_resource(sql=Field(str, default_value='select 1'),
                                                   db=Field(str, default_value='postgres'),
                                                   ),
                   "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"})},
    name='service__dwh_postgres_exec_sql_code',
)
def dwh_postgres_exec_sql_code_job():
    dwh_postgres_exec_sql_code_op()
