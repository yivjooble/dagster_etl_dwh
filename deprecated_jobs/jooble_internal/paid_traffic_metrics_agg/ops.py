...
TABLE_NAME = "paid_traffic_metrics_agg"
SCHEMA = "aggregation"
DATE_COLUMN = "date"
...
def paid_traffic_metrics_agg_get_sqlinstance(context):
    """
...
def paid_traffic_metrics_agg_query_on_db(context, sql_instance_country_query: dict):
    """
...
@job(
    config=job_config,
    resource_defs={
        "globals": make_values_resource(reload_date_start=Field(str, default_value=SEVENTH_DATE),
    ...
    metadata={
        "input_date": f"{SEVENTH_DATE} - {YESTERDAY_DATE}",
...
def paid_traffic_metrics_agg_job():
    instances = paid_traffic_metrics_agg_get_sqlinstance()
    instances.map(paid_traffic_metrics_agg_query_on_db)
