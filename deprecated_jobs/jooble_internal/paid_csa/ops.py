...
@op(out=DynamicOut())
def paid_csa_get_sqlinstance(context):
    '''
...
def paid_csa_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def paid_csa_job():
    instances = paid_csa_get_sqlinstance()
    instances.map(paid_csa_query_on_db)
