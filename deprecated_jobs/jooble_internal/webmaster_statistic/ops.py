...
TABLE_NAME = "webmaster_statistic"
SCHEMA = "traffic"
COUNTRY_COLUMN = "country_id"
DATE_COLUMN = "date"
FOURTH_DAY = (datetime.now() - timedelta(days=4)).date().isoformat()  # 4 days ago from today due to data lag
...
def webmaster_statistic_get_sql_instance(context):
    """
    ...
    for sql_instance in Operations.generate_sql_instance(
            ...
            db_name="SearchConsole",
            query=GITLAB_SQL_Q):
        yield sql_instance


@op(required_resource_keys={"globals"}, retry_policy=retry_policy)
def webmaster_statistic_query_on_db(context, sql_instance_country_query: dict):
    """
    ...
    to_sqlcode_tbl_stat = f"{country_code}_webmaster_statistic"
...
@job(
    ...
    resource_defs={
        "globals": make_values_resource(reload_countries=Field(list, default_value=all_countries_list),
                                        reload_date_start=Field(str, default_value=FOURTH_DAY),
                                        reload_date_end=Field(str, default_value=FOURTH_DAY),),
        "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"}),
    },
    name=generate_job_name(TABLE_NAME),
    tags={"data_model": f"{SCHEMA}"},
    metadata={
        "input_date": f"{FOURTH_DAY} - {FOURTH_DAY}",
...
def webmaster_statistic_job():
    instances = webmaster_statistic_get_sql_instance()
    instances.map(webmaster_statistic_query_on_db)
