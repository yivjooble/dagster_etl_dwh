...
TABLE_NAME = "m_ea_sales_usd_fakevsreal_revenue"
...
@op(out=DynamicOut())
def m_ea_sales_usd_fakevsreal_revenue_get_sqlinstance(context):
    """
...
@op(out=Out(List[str]))
def m_ea_sales_usd_fakevsreal_revenue_query_on_db(
...
@op
def m_ea_sales_usd_fakevsreal_revenue_truncate_table(context, query_result_file_paths):
...
@op(out=DynamicOut())
def m_ea_sales_usd_fakevsreal_revenue_collect_file_paths(context, file_paths_after_truncation):
    """
    Args:
...
@op
def m_ea_sales_usd_fakevsreal_revenue_save_to_dwh(context, params: dict):
    try:
...
def m_ea_sales_usd_fakevsreal_revenue_job():
    db_instances = m_ea_sales_usd_fakevsreal_revenue_get_sqlinstance()
    query_result_file_paths = db_instances.map(m_ea_sales_usd_fakevsreal_revenue_query_on_db).collect()
    file_paths_after_truncation = m_ea_sales_usd_fakevsreal_revenue_truncate_table(query_result_file_paths)
    file_paths_dynamic = m_ea_sales_usd_fakevsreal_revenue_collect_file_paths(file_paths_after_truncation)
    file_paths_dynamic.map(m_ea_sales_usd_fakevsreal_revenue_save_to_dwh)
