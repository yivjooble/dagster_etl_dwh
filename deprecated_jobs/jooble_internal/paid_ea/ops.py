...
@op(out=DynamicOut())
def paid_ea_get_sqlinstance(context):
    '''
...
def paid_ea_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def paid_ea_job():
    instances = paid_ea_get_sqlinstance()
    instances.map(paid_ea_query_on_db)
