...
from ..utils.rplc_config import clusters, map_country_code_to_id
...
TABLE_NAME = "adsense_version_daily"
SCHEMA = "aggregation"
DELETE_DATE_DIFF_COLUMN = "action_datediff"
DELETE_COUNTRY_COLUMN = "country_id"
YESTERDAY_DATE = (datetime.now().date() - timedelta(1)).strftime('%Y-%m-%d')
PATH_TO_DATA = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
PROCEDURE_CALL = "call an.prc_adsense_version_daily(%s);"
...
CUTSTOM_COUNTRIES_LIST = [
    'ua', 'de', 'uk', 'fr', 'ca', 'us', 'id', 'hu', 'ro', 'es', 'at', 'be', 'br', 'ch', 'cz', 'in', 'it',
...
def adsense_version_daily_get_sqlinstance(context):
    '''
...
def adsense_version_daily_launch_query_on_db(context, sql_instance_country_query: dict):
    ...
    for date in sql_instance_country_query['date_range']:
        ...
        start_query_on_rplc_db(context, local_sql_instance_country_query)
        ...
        local_sql_instance_country_query['query'] = select_q
        file_path = create_rplc_df(context, PATH_TO_DATA, local_sql_instance_country_query)
        ...
        if file_path.endswith(".pkl"):
            with gzip.open(file_path, 'rb') as f:
                ...
                if country_df.empty:
                    ...
                    delete_data_from_dwh_table(context, SCHEMA, TABLE_NAME, DELETE_COUNTRY_COLUMN,
                                               DELETE_DATE_DIFF_COLUMN, country_db, operation_date_diff, country_id)
                    save_to_dwh(country_df, TABLE_NAME, SCHEMA)
                    context.log.info(f'|{country_db.upper()}|: Successfully saved df to dwh.')


@job(config=job_config,
     resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=CUTSTOM_COUNTRIES_LIST),
...
def adsense_version_daily_job():
    instances = adsense_version_daily_get_sqlinstance()
    instances.map(adsense_version_daily_launch_query_on_db).collect()
