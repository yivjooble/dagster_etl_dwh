select (select case when (select current_database()) = 'ua' then 1 when (select current_database()) = 'de'
                ...
                then 71 end) as country,
	   id_account,
	   date_diff,
	   date,
	   id_session
from public.session_account
where date_diff between %(datediff)s and %(datediff)s
;