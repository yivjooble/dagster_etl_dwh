...
TABLE_NAME = "freecv_main_metrics"
SCHEMA = "kpav"
...
@op(out=Out(str))
def freecv_main_metrics_get_sql_query(context) -> str:
...
def freecv_main_metrics_get_sqlinstance(context, query):
    '''
...
def freecv_main_metrics_launch_query_on_db(context, sql_instance_country_query: dict) -> str:
    '''
...
@op(out=Out(str))
def freecv_main_metrics_get_sql_to_create_rplc_df(context, result) -> str:
    '''Get sql query from .sql file.'''
...
@op(out=DynamicOut(),
    required_resource_keys={'globals'})
def freecv_main_metrics_launch_df_creation(context, query):
...
@op(out=Out(str), retry_policy=retry_policy)
def freecv_main_metrics_launch_query_to_create_df(context, sql_instance_country_query: dict) -> str:
    '''
    Launch query on each instance.
    '''
...
@op(ins={"file_paths": In(List[str])},
    required_resource_keys={'globals'})
def freecv_main_metrics_delete_history_data(context, **kwargs):
    launch_countries = context.resources.globals["reload_countries"]
    # if 'datediff' or 'date' format
    is_datediff = True
    launch_datediff_start = get_datediff(context.resources.globals["reload_date_start"]) if is_datediff else \
...
def freecv_main_metrics_save_df_to_dwh(context, file_paths, delete):
    try:
        all_dfs_list = []
...
def freecv_main_metrics_job():
    # start procedure on replica
    call_procedure_query = freecv_main_metrics_get_sql_query()
    replica_instances = freecv_main_metrics_get_sqlinstance(call_procedure_query)
    collect_result = replica_instances.map(freecv_main_metrics_launch_query_on_db).collect()
    # create df from procedure result table
    sql_create_df = freecv_main_metrics_get_sql_to_create_rplc_df(collect_result)
    df_creation_instances = freecv_main_metrics_launch_df_creation(sql_create_df)
    file_paths = df_creation_instances.map(freecv_main_metrics_launch_query_to_create_df).collect()
    # delete history from dwh
    delete = freecv_main_metrics_delete_history_data(file_paths)
    # save df to dwh
    freecv_main_metrics_save_df_to_dwh(file_paths, delete)
