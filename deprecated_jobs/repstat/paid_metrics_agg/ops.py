...
PROCEDURE_CALL = "call an.prc_paid_metrics_agg(%s);"
...
def paid_metrics_agg_get_sqlinstance(context):
    '''
...
def paid_metrics_agg_query_on_db(context, sql_instance_country_query: dict):
...
def paid_metrics_agg_job():
    instances = paid_metrics_agg_get_sqlinstance()
    instances.map(paid_metrics_agg_query_on_db).collect()
