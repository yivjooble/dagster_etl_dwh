...
TWO_DAYS_AGO = (datetime.now().date() - timedelta(2)).strftime('%Y-%m-%d')
SEVENTH_DATE = (datetime.now().date() - timedelta(7)).strftime('%Y-%m-%d')
PATH_TO_DATA = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
JOB_PREFIX = job_prefix()
PROCEDURE_CALL = "call an.prc_email_metric_daily(%s);"
...
custom_countries_list = [
    'ua', 'de', 'uk', 'fr', 'ca', 'us',
    # 'id', 'nz', 'mx',
    'pl', 'hu', 'ro', 'es', 'at', 'be', 'br', 'ch', 'cz', 'in', 'it',
    'nl', 'tr', 'cl', 'co', 'gr', 'sk', 'th', 'tw', 'bg', 'hr', 'kz', 'no_', 'rs', 'se', 'ng', 'ar',
...
@op(retry_policy=retry_policy,
    required_resource_keys={'globals'})
def email_metric_daily_launch_query_on_db(context, sql_instance_country_query: dict):
...
@job(config=job_config,
     ...
     name=JOB_PREFIX + TABLE_NAME + "_yesterday",
...
def email_metric_daily_job_yesterday():
    """
    Job to start procedure on replica, create dataframe from procedure result and save it to dwh.
    """
    # start procedure on replica
    replica_instances = email_metric_daily_get_sql_instance()
    replica_instances.map(email_metric_daily_launch_query_on_db).collect()
...
@job(config=job_config,
     resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=custom_countries_list),
                                                    ...
                                                    reload_date_end=Field(str, default_value=TWO_DAYS_AGO),
                                                    is_datediff=Field(bool, default_value=True)),
                    "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"})},
     name=JOB_PREFIX + TABLE_NAME + "_last_six_days",
     description=f'{SCHEMA}.{TABLE_NAME}',
     tags={"data_model": f"{SCHEMA}"},
     metadata={
         "input_date": f"{YESTERDAY_DATE}",
         "GITLAB_SQL_URL": f"{GITLAB_DDL_URL}",
         "destination_db": "dwh",
         "target_table": f"{SCHEMA}.{TABLE_NAME}",
     }
     )
def email_metric_daily_job_last_six_days():
    """
    Job to start procedure on replica, create dataframe from procedure result and save it to dwh.
    """
    # start procedure on replica
