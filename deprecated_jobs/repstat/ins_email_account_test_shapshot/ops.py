...
TABLE_NAME = "prc_insert_email_account_test_snapshot"
SCHEMA = "an"
...
@op(out=Out(str))
def prc_insert_email_account_test_snapshot_get_sql_query(context) -> str:
...
def prc_insert_email_account_test_snapshot_get_sqlinstance(context, query):
    '''
...
def prc_insert_email_account_test_snapshot_launch_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def prc_insert_email_account_test_snapshot_first_part_job():
    # start procedure on replica
    call_procedure_query = prc_insert_email_account_test_snapshot_get_sql_query()
    replica_instances = prc_insert_email_account_test_snapshot_get_sqlinstance(call_procedure_query)
    replica_instances.map(prc_insert_email_account_test_snapshot_launch_query_on_db)
...
@job(config=snapshot_job_config,
    ...
    description=f'{SCHEMA}.{TABLE_NAME}')
def prc_insert_email_account_test_snapshot_second_part_job():
    # start procedure on replica
    call_procedure_query = prc_insert_email_account_test_snapshot_get_sql_query()
    replica_instances = prc_insert_email_account_test_snapshot_get_sqlinstance(call_procedure_query)
    replica_instances.map(prc_insert_email_account_test_snapshot_launch_query_on_db)