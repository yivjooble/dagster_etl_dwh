...
@op(out=Out(str))
def session_get_sql_query(context) -> str:
...
def session_get_sqlinstance(context, query):
    '''
...
def session_launch_query_on_db(context, sql_instance_country_query: dict) -> str:
    '''
...
@op(ins={"file_paths": In(List[str])},
    required_resource_keys={'globals'})
def session_delete_history_data(context, **kwargs):
...
def session_save_df_to_dwh(context, file_paths, delete):
    try:
        all_dfs_list = []
...
def session_delete_old_data(context, save_result):
    # Get sql query from .sql file.
...
def session_job():
    query = session_get_sql_query()
    instances = session_get_sqlinstance(query)
    file_paths = instances.map(session_launch_query_on_db).collect()
    delete = session_delete_history_data(file_paths)
    save_result = session_save_df_to_dwh(file_paths, delete)
    # session_delete_old_data(save_result)