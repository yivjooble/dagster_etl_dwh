...
@op(out=Out(str))
def snap_job_history_get_sql_query(context) -> str:
...
def snap_job_history_get_sqlinstance(context, query):
    '''
...
def snap_job_history_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def snap_job_history_job():
    """
    This function executes the snap_job_history job.

    It retrieves a SQL query using the snap_job_history_get_sql_query function,
    gets SQL instances using the snap_job_history_get_sqlinstance function,
    and maps the snap_job_history_query_on_db function to each instance.
    """
    query = snap_job_history_get_sql_query()
    instances = snap_job_history_get_sqlinstance(query)
    instances.map(snap_job_history_query_on_db)