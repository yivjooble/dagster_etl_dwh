...
TABLE_NAME = "field_from_source_daily"
SCHEMA = "imp"
DATE_COLUMN = "job_date_created"
...
def field_from_source_daily_get_sql_instance(context):
    """
...
def field_from_source_daily_query_on_db(context, sql_instance_country_query: dict):
    """
    ...
    try:
        ...
        for date in sql_instance_country_query['date_range']:
            ...
            DwhOperations.save_to_dwh_upsert(context, SCHEMA, TABLE_NAME, df=first_chunk)

            # Save the remaining chunks
            for chunk in chunk_generator:
                DwhOperations.save_to_dwh_upsert(context, SCHEMA, TABLE_NAME, df=chunk)
...
def field_from_source_daily_job():
    instances = field_from_source_daily_get_sql_instance()
    instances.map(field_from_source_daily_query_on_db).collect()
