...
TABLE_NAME = "jobs_stat_daily"
SCHEMA = "aggregation"
COUNTRY_COLUMN = "id_country"
...
def jobs_stat_daily_get_sqlinstance(context):
    '''
...
def jobs_stat_daily_query_on_db(context, sql_instance_country_query: dict):
    '''
    ...
    try:
        ...
        DwhOperations.save_to_dwh_pandas(context=context, df=first_chunk, schema=SCHEMA, table_name=TABLE_NAME)

        # Save the remaining chunks
        for chunk in chunk_generator:
            DwhOperations.save_to_dwh_pandas(context=context, df=chunk, schema=SCHEMA, table_name=TABLE_NAME)
...
def jobs_stat_daily_job():
    instances = jobs_stat_daily_get_sqlinstance()
    instances.map(jobs_stat_daily_query_on_db)
