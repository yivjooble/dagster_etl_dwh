...
TABLE_NAME = "session_away_message"
SCHEMA = "imp"
...
@op(out=Out(str))
def session_away_message_get_sql_query(context) -> str:
...
def session_away_message_get_sqlinstance(context, query):
    '''
...
def session_away_message_launch_query_on_db(context, sql_instance_country_query: dict) -> str:
    '''
...
@op(ins={"file_paths": In(List[str])},
    required_resource_keys={'globals'})
def session_away_message_delete_history_data(context, **kwargs):
...
def session_away_message_save_df_to_dwh(context, file_paths, delete):
    try:
        all_dfs_list = []
...
def session_away_message_delete_old_data(context, save_result):
    # Get sql query from .sql file.
...
def session_away_message_job():
    query = session_away_message_get_sql_query()
    instances = session_away_message_get_sqlinstance(query)
    file_paths = instances.map(session_away_message_launch_query_on_db).collect()
    delete = session_away_message_delete_history_data(file_paths)
    save_result = session_away_message_save_df_to_dwh(file_paths, delete)
    # session_away_message_delete_old_data(save_result)