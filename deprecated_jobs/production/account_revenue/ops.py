...
SCHEMA = "aggregation"
DELETE_DATE_DIFF_COLUMN = "load_date"
...
@op(out=Out(str))
def account_revenue_get_sql_query(context) -> str:
...
def account_revenue_get_sqlinstance(context, query):
    '''
...
def account_revenue_query_on_db(context, sql_instance_country_query: dict) -> list:
    '''
    ...
    for date in sql_instance_country_query['date_range']:
        ...
        sql_instance_country_query['to_sqlcode_date_date_start'] = date.strftime('%Y-%m-%d')
...
@op(ins={"file_paths": In(List)},
    required_resource_keys={'globals'})
def account_revenue_delete_history_data(context, **kwargs):
    launch_countries = context.resources.globals["reload_countries"]
    date_range = pd.date_range(pd.to_datetime(context.resources.globals["reload_date_start"]),
                               pd.to_datetime(context.resources.globals["reload_date_end"]))
    ...
    context.log.info("===== START DELETION PROCESS")
...
def account_revenue_save_df_to_dwh(context, file_paths, delete):
    try:
        all_dfs_list = []
        for f in file_paths:
            for file_path in f:
...
def account_revenue_job():
    query = account_revenue_get_sql_query()
    instances = account_revenue_get_sqlinstance(query)
    file_paths = instances.map(account_revenue_query_on_db).collect()
    delete = account_revenue_delete_history_data(file_paths)
    account_revenue_save_df_to_dwh(file_paths, delete)
