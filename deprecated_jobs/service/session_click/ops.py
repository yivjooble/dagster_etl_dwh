...
from etl_production.utils.prod_job_config import retry_policy, job_config
from etl_production.utils.prod_sqlinstances import SqlInstanceList, map_country_code_to_id, all_countries_list
from etl_production.utils.prod_db_operations import start_query_on_prod_db
from etl_production.utils.date_format_settings import get_datediff
from etl_production.utils.dwh_db_operations import delete_data_from_dwh_table, save_to_dwh, execute_on_dwh
from etl_production.utils.utils import delete_pkl_files, map_country_to_id
from etl_production.utils.messages import send_dwh_alert_slack_message

TABLE_NAME = "session_click"
SCHEMA = "imp"
...
@op(out=Out(str))
def session_click_get_sql_query(context) -> str:
...
def session_click_get_sqlinstance(context, query):
    '''
...
def session_click_launch_query_on_db(context, sql_instance_country_query: dict) -> str:
    '''
...
@op(ins={"file_paths": In(List[str])},
    required_resource_keys={'globals'})
def session_click_delete_history_data(context, **kwargs):
    launch_countries = context.resources.globals["reload_countries"]
    # if 'datediff' or 'date' format
    is_datediff = context.resources.globals["is_datediff"]
    launch_datediff_start = get_datediff(context.resources.globals["reload_date_start"]) if is_datediff else \
...
def session_click_save_df_to_dwh(context, file_paths, delete):
    try:
...
@job(config=job_config,
     ...
     name='service__reload_' + TABLE_NAME,
     description=f'{SCHEMA}.{TABLE_NAME}')
def session_click_job():
    query = session_click_get_sql_query()
    instances = session_click_get_sqlinstance(query)
    file_paths = instances.map(session_click_launch_query_on_db).collect()
    delete = session_click_delete_history_data(file_paths)
    session_click_save_df_to_dwh(file_paths, delete)
