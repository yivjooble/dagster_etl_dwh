...
logging.basicConfig(filename='logs/net_revenue_month_forecast.log',
                    filemode='a',
...
try:
    ...
    query = """Select cast(dv_revenue_by_placement_and_src.date as date) as date, 
                sum(revenue_usd) - sum(cost_usd) as revenue
    ...
    result_df_1_row = pred_uc.predicted_mean.to_frame('net_revenue_forecast')
    result_df_2_row = predict_dy_ci
    result_df_2_row = result_df_2_row.rename({"lower y" : "net_lower_y", "upper y" : "net_upper_y"}, axis="columns")
    ...
    cur = conn2.cursor()
    cur.execute("delete from aggregation.net_revenue_month_forecast where date::date >= current_date;")

    conn2.commit()
    conn2.close()
    logging.info(f"deleted (date::date >= current_date) --> aggregation.net_revenue_month_forecast")
