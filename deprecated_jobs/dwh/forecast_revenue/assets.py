...
@asset(group_name='revenue_month_forecast',
       key_prefix='revenue_month_forecast_assets')
def launch_revenue_model(context) -> pd.DataFrame:
    context.log.info(f"Loading started\n"
                f"Creating dataframe from sql-query")
    plt.style.use('fivethirtyeight')
    
    path_to_sql_file = os.path.join(os.path.dirname(__file__), os.path.join("sql", "forecast_revenue.sql"))
...
def save_to_dwh(context, launch_revenue_model):
    # Save to dwh
