...
TABLE_NAME = "project_tracking_setting_test"
SCHEMA = "dwh_test"
...
GITLAB_SQL_Q, GITLAB_SQL_URL = fetch_gitlab_data(
    config_key="default",
    dir_name='dimension',
    file_name='project_tracking_setting',
)


@op(out=DynamicOut(), required_resource_keys={"globals"})
def project_tracking_setting_test_get_sqlinstance(context):
    """
    Loop over prod sql instances and create output dictionary with data to start on a separate instance.
    Args:
        context: The context object.
        truncate: link to the truncate op.
    Output:
...
def project_tracking_setting_test_query_on_db(context, sql_instance_country_query: dict):
    """
...
def project_tracking_setting_test_job():
    instances = project_tracking_setting_test_get_sqlinstance()
    instances.map(project_tracking_setting_test_query_on_db)
