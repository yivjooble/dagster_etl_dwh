...
TABLE_NAME = "session_daily_agg"
SCHEMA = "aggregation"
DATE_DIFF_COLUMN = "action_datediff"
...
def session_daily_agg_get_sqlinstance(context):
    """
...
def session_daily_agg_query_on_db(context, sql_instance_country_query: dict):
    ...
    try:
        ...
        DwhOperations.save_to_dwh_pandas(context=context, df=first_chunk, schema=SCHEMA, table_name=TABLE_NAME, destination_db=destination_db)

        # Save the remaining chunks
        for chunk in chunk_generator:
            DwhOperations.save_to_dwh_pandas(context=context, df=chunk, schema=SCHEMA, table_name=TABLE_NAME, destination_db=destination_db)
...
def session_daily_agg_job():
    instances = session_daily_agg_get_sqlinstance()
    instances.map(session_daily_agg_query_on_db)
