...
def funnel_agg_get_sqlinstance(context):
    '''
...
def funnel_agg_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def funnel_agg_job():
    instances = funnel_agg_get_sqlinstance()
    instances.map(funnel_agg_query_on_db).collect()
