...
TABLE_NAME = "bot_jdp_away_clicks_agg"
SCHEMA = "aggregation"
DATE_COLUMN = "date"
...
CUSTOM_COUNTRY_LIST = ['ua', 'de', 'uk', 'fr', 'ca', 'us', 'hu', 'ro', 'at', 'ch']


@op(out=DynamicOut(), required_resource_keys={'globals'})
def bot_jdp_away_clicks_agg_get_sqlinstance(context):
    '''
...
def bot_jdp_away_clicks_agg_query_on_db(context, sql_instance_country_query: dict):
    '''
    ...
    try:
        ...
        sql_instance_country_query['to_sqlcode_date_or_datediff_start'] = get_datediff(operation_date_start)
        sql_instance_country_query['to_sqlcode_date_or_datediff_end'] = get_datediff(operation_date_end)
...
def PRD_bot_jdp_away_clicks_agg_job():
    instances = bot_jdp_away_clicks_agg_get_sqlinstance()
    instances.map(bot_jdp_away_clicks_agg_query_on_db)
