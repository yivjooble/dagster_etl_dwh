...
CUSTOM_COUNTRIES = ['ua', 'fr', 'pl', 'hu', 'ro', 'es', 'it', 'co', 'br']


@op(out=DynamicOut(), required_resource_keys={'globals'})
def email_account_test_get_sqlinstance(context):
    """
    ...
    """
    # Delete data from dwh table older than 15 days
    deletion_date_diff = get_datediff(YESTERDAY_DATE) - 14
    query = f"DELETE FROM {SCHEMA}.{TABLE_NAME} WHERE {DATE_DIFF_COLUMN} < %s"

    context.log.info(f"Deleting data from dwh table older than 15 days")
    DwhOperations.execute_on_dwh(context=context,
                                 query=query,
                                 params=(deletion_date_diff,))
...
def email_account_test_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def email_account_test_job():
    instances = email_account_test_get_sqlinstance()
    instances.map(email_account_test_query_on_db)
