...
TABLE_NAME = "auction_click_statistic"
...
def auction_click_statistic_get_sqlinstance(context):
    """
...
def auction_click_statistic_query_on_db(context, sql_instance_country_query: dict):
    """
    ...
    try:
        ...
        sql_instance_country_query['to_sqlcode_date_or_datediff_start'] = operation_date_start
        sql_instance_country_query['to_sqlcode_date_or_datediff_end'] = operation_date_end
...
def auction_click_statistic_job():
    instances = auction_click_statistic_get_sqlinstance()
    instances.map(auction_click_statistic_query_on_db)
...
def auction_click_statistic_reload_us_job():
    instances = auction_click_statistic_get_sqlinstance()
    instances.map(auction_click_statistic_query_on_db)
...
@job(
    ...
    resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=all_countries_list),
                                                   ...
                                                   destination_db=Field(str, default_value="both"),),
                   "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"})},
    name=generate_job_name(TABLE_NAME, "_reload_last_month"),
    tags={"data_model": f"{SCHEMA}"},
    metadata={
...
def auction_click_statistic_reload_last_month_job():
    instances = auction_click_statistic_get_sqlinstance()
    instances.map(auction_click_statistic_query_on_db)
