...
def u_traffic_source_get_sqlinstance(context):
    """
...
def u_traffic_source_query_on_db(context, sql_instance_country_query: dict):
    """
    ...
    try:
        ...
        DwhOperations.save_to_dwh_pandas(context, schema=SCHEMA, table_name=TABLE_NAME, df=first_chunk, destination_db=destination_db)
...
def u_traffic_source_job():
    instances = u_traffic_source_get_sqlinstance()
    instances.map(u_traffic_source_query_on_db)
