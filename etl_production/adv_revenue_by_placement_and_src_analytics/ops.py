...
def adv_revenue_by_placement_and_src_analytics_get_sqlinstance(context):
    '''
...
def adv_revenue_by_placement_and_src_analytics_query_on_db(context, sql_instance_country_query: dict):
    '''
    ...
    try:
        ...
        for date in sql_instance_country_query['date_range']:
            ...
            operation_month_date_diff = context.resources.globals["reload_month_date_diff"]

            local_sql_instance_country_query['to_sqlcode_date_or_datediff_start'] = operation_date_diff
            local_sql_instance_country_query['to_sqlcode_month_date_diff'] = operation_month_date_diff
...
@job(
    ...
    resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=all_countries_list),
                                                   ...
                                                   reload_month_date_diff=Field(int, default_value=get_first_day_of_month_as_int()),
...
def adv_revenue_by_placement_and_src_analytics_agg_job():
    instances = adv_revenue_by_placement_and_src_analytics_get_sqlinstance()
    instances.map(adv_revenue_by_placement_and_src_analytics_query_on_db).collect()
