...
def jdp_away_clicks_agg_query_on_db(context, sql_instance_country_query: dict):
    """
...
def jdp_away_clicks_agg_job():
    instances = jdp_away_clicks_agg_get_sql_instance()
    instances.map(jdp_away_clicks_agg_query_on_db).collect()
...
@job(
    ...
    resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=all_countries_list),
                                                   ...
                                                   destination_db=Field(str, default_value="both")),
                   "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"})},
    name=generate_job_name(TABLE_NAME, '_last_seven_days'),
    tags={"data_model": f"{SCHEMA}"},
    metadata={
        "input_date": f"{YESTERDAY_DATE} - {SEVENTH_DATE}",
        "gitlab_sql_url": f"{GITLAB_SQL_URL}",
        "destination_db": "dwh, cloudberry, both",
        "target_table": f"{SCHEMA}.{TABLE_NAME}",
    },
...
def jdp_away_clicks_agg_last_seven_days():
    instances = jdp_away_clicks_agg_get_sql_instance()
    instances.map(jdp_away_clicks_agg_query_on_db).collect()
