...
TABLE_NAME = "mobile_session_feature"
SCHEMA = "mobile_app"
DATE_DIFF_COLUMN = "session_feature_datediff"
COUNTRY_COLUMN = "country_id"

YESTERDAY_DATE = (datetime.now().date() - timedelta(1)).strftime('%Y-%m-%d')
PROCEDURE_CALL = "call an.prc_mobile_session_feature(%s);"
...
def mobile_session_feature_get_sqlinstance(context):
    '''
...
def mobile_session_feature_query_on_db(context, sql_instance_country_query: dict):
...
def mobile_session_feature_job():
    instances = mobile_session_feature_get_sqlinstance()
    instances.map(mobile_session_feature_query_on_db).collect()
