...
TABLE_NAME = "recommendations_agg"
SCHEMA = "aggregation"
...
PROCEDURE_CALL = "call an.prc_recommendations_agg(%s);"
...
def recommendations_agg_get_sqlinstance(context):
    '''
...
def recommendations_agg_query_on_db(context, sql_instance_country_query: dict):
...
def recommendations_agg_job():
    instances = recommendations_agg_get_sqlinstance()
    instances.map(recommendations_agg_query_on_db).collect()
