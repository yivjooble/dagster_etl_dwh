...
PROCEDURE_CALL = "call an.prc_clicks_collars(%s);"
...
def clicks_collars_get_sqlinstance(context):
    '''
...
def clicks_collars_query_on_db(context, sql_instance_country_query: dict):
...
def clicks_collars_job():
    instances = clicks_collars_get_sqlinstance()
    instances.map(clicks_collars_query_on_db).collect()
