...
TABLE_NAME = "email_spam_report_agg"
SCHEMA = "aggregation"
...
PROCEDURE_CALL = "call an.prc_email_spam_report_agg(%s);"
...
def email_spam_report_agg_get_sqlinstance(context):
    '''
...
def email_spam_report_agg_query_on_db(context, sql_instance_country_query: dict):
...
def email_spam_report_agg_job():
    instances = email_spam_report_agg_get_sqlinstance()
    instances.map(email_spam_report_agg_query_on_db).collect()
