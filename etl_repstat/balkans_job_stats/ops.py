...
PROCEDURE_CALL = "call an.prc_balkans_job_stats(%s);"
...
def balkans_job_stats_get_sqlinstance(context):
    '''
...
def balkans_job_stats_query_on_db(context, sql_instance_country_query: dict):
...
def balkans_job_stats_job():
    instances = balkans_job_stats_get_sqlinstance()
    instances.map(balkans_job_stats_query_on_db).collect()
