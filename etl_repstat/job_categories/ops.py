...
PROCEDURE_CALL = "call an.prc_job_categories_agg(%s);"
...
CUSTOM_COUNTRY_LIST = [
    'ua', 'de', 'uk', 'fr', 'ca', 'us', 'hu', 'ro', 'at', 'ch', 'pl', 'au',
    'be', 'nl', 'cz', 'it', 'es'
]


@op(out=DynamicOut(), required_resource_keys={'globals'})
def job_categories_get_sqlinstance(context):
    '''
...
def job_categories_query_on_db(context, sql_instance_country_query: dict):
...
def job_categories_job():
    instances = job_categories_get_sqlinstance()
    instances.map(job_categories_query_on_db).collect()
