...
PROCEDURE_CALL = "call an.prc_session_abtest_agg_new(%s);"
...
def session_abtest_new_get_sqlinstance(context):
    '''
...
@job(
    ...
    name=generate_job_name(TABLE_NAME, "_new_yesterday_date"),
...
def session_abtest_agg_new_job_yesterday_date():
    instances = session_abtest_new_get_sqlinstance()
    instances.map(session_abtest_new_query_on_db).collect()
...
@job(
    ...
    resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=all_countries_list),
                   ...
                   "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"})},
    name=generate_job_name(TABLE_NAME, "_new_current_date"),
    tags={"data_model": f"{SCHEMA}"},
    metadata={
...
def session_abtest_agg_new_job_current_date():
    instances = session_abtest_new_get_sqlinstance()
    instances.map(session_abtest_new_query_on_db).collect()
