...
PROCEDURE_CALL = "call an.prc_mobile_app_push_metrics(%s);"
...
def push_metrics_get_sql_instance(context):
    '''
...
def mobile_app_push_metrics_job():
    instances = push_metrics_get_sql_instance()
    results = instances.map(push_metrics_query_on_db).collect()
    push_metrics_refresh_tableau_object(results)
