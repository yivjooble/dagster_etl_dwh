...
PROCEDURE_CALL = "call an.prc_itc_jdp(%s);"
...
def itc_jdp_get_sqlinstance(context):
    '''
...
def itc_jdp_query_on_db(context, sql_instance_country_query: dict):
...
def itc_jdp():
    instances = itc_jdp_get_sqlinstance()
    instances.map(itc_jdp_query_on_db).collect()
