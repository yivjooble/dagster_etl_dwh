...
TABLE_NAME = "snap_job_history"
SCHEMA = "an"

YESTERDAY_DATE = (datetime.now().date() - timedelta(1)).strftime('%Y-%m-%d')
CURRENT_DATE = datetime.now().date().strftime('%Y-%m-%d')

PROCEDURE_CALL = "call an.prc_insert_job_history_snapshot(%s);"
...
def prc_insert_job_history_snapshot_get_sqlinstance(context):
...
def prc_insert_job_history_snapshot_query_on_db(context, sql_instance_country_query: dict):
...
def prc_insert_job_history_snapshot_job_yesterday_date():
    instances = prc_insert_job_history_snapshot_get_sqlinstance()
    instances.map(prc_insert_job_history_snapshot_query_on_db)
...
def prc_insert_job_history_snapshot_job_current_date():
    instances = prc_insert_job_history_snapshot_get_sqlinstance()
    instances.map(prc_insert_job_history_snapshot_query_on_db)
