...
PROCEDURE_CALL = "call an.prc_cv_information_agg(%s);"
...
def cv_information_agg_get_sqlinstance(context):
    '''
...
def cv_information_agg_query_on_db(context, sql_instance_country_query: dict):
    ...
    for date in sql_instance_country_query['date_range']:
        ...
        delete_q, delete_q_url = fetch_gitlab_data(
            config_key="repstat",
            dir_name=PROC_NAME_PARSED,
            file_name='history_delete')

        context.log.debug(f'Gitlab delete-query url:\n{delete_q_url}')

        sql = delete_q.format(
            SCHEMA=SCHEMA,
            TABLE_NAME=TABLE_NAME,
            delete_country_column=COUNTRY_COLUMN,
            delete_date_diff_column=DATE_COLUMN,
            id_list=country_id,
            delete_date=operation_date
        )

        try:
            DwhOperations.execute_on_dwh(context=context, query=sql, destination_db=destination_db)
...
def cv_information_agg_job():
    instances = cv_information_agg_get_sqlinstance()
    instances.map(cv_information_agg_query_on_db).collect()
