...
TABLE_NAME = "mobile_session_feature_action"
SCHEMA = "mobile_app"
DATE_DIFF_COLUMN = "session_feature_action_datediff"
COUNTRY_COLUMN = "country_id"

YESTERDAY_DATE = (datetime.now().date() - timedelta(1)).strftime('%Y-%m-%d')
PROCEDURE_CALL = "call an.prc_mobile_session_feature_action(%s);"
...
def mobile_session_feature_action_get_sqlinstance(context):
    '''
...
def mobile_session_feature_action_query_on_db(context, sql_instance_country_query: dict):
...
def mobile_session_feature_action_job():
    instances = mobile_session_feature_action_get_sqlinstance()
    instances.map(mobile_session_feature_action_query_on_db).collect()
