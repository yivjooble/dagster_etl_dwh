...
TABLE_NAME = "email_abtest_agg_by_account"
SCHEMA = "an"

YESTERDAY_DATE = (datetime.now().date() - timedelta(1)).strftime('%Y-%m-%d')
CURRENT_DATE = datetime.now().date().strftime('%Y-%m-%d')

PROCEDURE_CALL = "call an.prc_email_abtest_agg_by_account(%s);"
...
def email_abtest_agg_by_account_get_sqlinstance(context):
    '''
...
@job(
    ...
    name=generate_job_name(TABLE_NAME, "_to_rpl_yesterday_date"),
...
def email_abtest_agg_by_account_job_yesterday_date():
    instances = email_abtest_agg_by_account_get_sqlinstance()
    instances.map(email_abtest_agg_by_account_query_on_db).collect()
...
@job(
    ...
    resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=all_countries_list),
                   ...
                   "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"})},
    name=generate_job_name(TABLE_NAME, "_to_rpl_current_date"),
    tags={"data_model": f"{SCHEMA}"},
    metadata={
...
def email_abtest_agg_by_account_job_current_date():
    instances = email_abtest_agg_by_account_get_sqlinstance()
    instances.map(email_abtest_agg_by_account_query_on_db).collect()
