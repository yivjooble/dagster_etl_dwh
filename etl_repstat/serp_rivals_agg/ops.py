...
PROCEDURE_CALL = "call an.prc_serp_rivals_agg(%s);"
...
def serp_rivals_agg_get_sqlinstance(context):
    '''
...
def serp_rivals_agg_query_on_db(context, sql_instance_country_query: dict):
...
def serp_rivals_agg_job():
    instances = serp_rivals_agg_get_sqlinstance()
    instances.map(serp_rivals_agg_query_on_db).collect()
