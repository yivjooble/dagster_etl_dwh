...
PROCEDURE_CALL = "call an.prc_search_agg(%s);"
...
def search_agg_get_sqlinstance(context):
    '''
...
def search_agg_query_on_db(context, sql_instance_country_query: dict):
...
def search_agg_abtest_agg_job():
    instances = search_agg_get_sqlinstance()
    instances.map(search_agg_query_on_db).collect()
