...
SCHEMA = "mobile_app"
DATE_DIFF_COLUMN = "load_datediff"
COUNTRY_COLUMN = "country_id"

YESTERDAY_DATE = (datetime.now().date() - timedelta(1)).strftime('%Y-%m-%d')

PROCEDURE_CALL = "call an.prc_mobile_app_business_metrics(%s);"
...
def business_metrics_get_sql_instance(context):
    '''
...
def business_metrics_query_on_db(context, sql_instance_country_query: dict):
    """
...
@op
def business_metrics_refresh_tableau_object(context, results):
    '''
...
def mobile_app_business_metrics_job():
    instances = business_metrics_get_sql_instance()
    results = instances.map(business_metrics_query_on_db).collect()
    business_metrics_refresh_tableau_object(results)
