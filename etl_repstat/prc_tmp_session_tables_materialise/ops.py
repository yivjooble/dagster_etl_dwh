...
PROCEDURE_CALL = "call an.prc_tmp_session_tables_materialise();"
...
def prc_tmp_session_tables_materialise_get_sqlinstance(context):
...
def prc_tmp_session_tables_materialise_query_on_db(context, sql_instance_country_query: dict):
...
@job(
    ...
    name=generate_job_name(PROC_NAME_PARSED),
    tags={"data_model": f"{SCHEMA}"},
    metadata={
        "gitlab_ddl_url": f"{GITLAB_DDL_URL}",
        "destination_db": "rpl",
        "target_tables": """[an.tmp_session_jdp],
                            [an.tmp_session_click],
                            [an.tmp_session_away]""",
    },
    description="""Run procedure [an.prc_tmp_session_tables_materialise] on replica.
                   Rewrite tables: [an.tmp_session_jdp], [an.tmp_session_click], [an.tmp_session_away]""",
)
def prc_tmp_session_tables_materialise_job():
    instances = prc_tmp_session_tables_materialise_get_sqlinstance()
    instances.map(prc_tmp_session_tables_materialise_query_on_db)
