...
TABLE_NAME = "letter_job_click"
SCHEMA = "aggregation"
DATE_DIFF_COLUMN = "job_click_datediff"
COUNTRY_COLUMN = "country_id"

YESTERDAY_DATE = (datetime.now().date() - timedelta(1)).strftime('%Y-%m-%d')

PROCEDURE_CALL = "call an.prc_letter_job_click(%s);"
...
def letter_job_click_get_sqlinstance(context):
    '''
...
def letter_job_click_query_on_db(context, sql_instance_country_query: dict):
...
def letter_job_click():
    instances = letter_job_click_get_sqlinstance()
    instances.map(letter_job_click_query_on_db).collect()
