...
TABLE_NAME = "click_metric_agg"
SCHEMA = "an"

YESTERDAY_DATE = (datetime.now().date() - timedelta(1)).strftime('%Y-%m-%d')
CURRENT_DATE = datetime.now().date().strftime('%Y-%m-%d')

PROCEDURE_CALL = "call an.prc_click_metric_agg(%s);"
...
def prc_click_metric_agg_job_yesterday_date():
    instances = prc_click_metric_agg_get_sqlinstance()
    instances.map(prc_click_metric_agg_query_on_db)
...
def prc_click_metric_agg_job_current_date():
    instances = prc_click_metric_agg_get_sqlinstance()
    instances.map(prc_click_metric_agg_query_on_db)
