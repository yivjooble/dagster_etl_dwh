...
PROCEDURE_CALL = "call an.prc_account_revenue(%s);"
...
def account_revenue_get_sqlinstance(context):
...
def account_revenue_query_on_db(context, sql_instance_country_query: dict):
...
@job(
    ...
    name=generate_job_name(TABLE_NAME, "_to_dwh"),
...
def account_revenue_job():
    instances = account_revenue_get_sqlinstance()
    instances.map(account_revenue_query_on_db).collect()
