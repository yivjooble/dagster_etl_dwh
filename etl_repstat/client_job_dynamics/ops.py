...
PROCEDURE_CALL = "call an.prc_client_job_dynamics(%s);"
...
def client_job_dynamics_get_sql_instance(context):
    '''
...
def client_job_dynamics_query_on_db(context, sql_instance_country_query: dict):
    """
...
def client_job_dynamics_job():
    instances = client_job_dynamics_get_sql_instance()
    instances.map(client_job_dynamics_query_on_db).collect()
