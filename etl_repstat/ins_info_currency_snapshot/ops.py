...
TABLE_NAME = "snap_info_currency"
SCHEMA = "an"

PROCEDURE_CALL = "call an.prc_insert_info_currency_snapshot();"
...
def prc_insert_info_currency_snapshot_get_sqlinstance(context):
...
def prc_insert_info_currency_snapshot_query_on_db(context, sql_instance_country_query: dict):
    ...
    context.log.info(f"--> Starting sql-script on: {sql_instance_country_query['db_name']}")
...
def prc_insert_info_currency_snapshot_job():
    instances = prc_insert_info_currency_snapshot_get_sqlinstance()
    instances.map(prc_insert_info_currency_snapshot_query_on_db)
