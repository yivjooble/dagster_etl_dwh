...
PROCEDURE_CALL = "call an.prc_action_agg(%s);"
...
def action_agg_get_sqlinstance(context):
    '''
...
def action_agg_query_on_db(context, sql_instance_country_query: dict):
...
def action_agg_job():
    instances = action_agg_get_sqlinstance()
    instances.map(action_agg_query_on_db).collect()
