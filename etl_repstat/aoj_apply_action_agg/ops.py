...
TABLE_NAME = "aoj_apply_action_agg"
SCHEMA = "aggregation"
...
PROCEDURE_CALL = "call an.prc_aoj_apply_action_agg(%s);"
...
def aoj_apply_action_agg_get_sqlinstance(context):
    '''
...
def aoj_apply_action_agg_query_on_db(context, sql_instance_country_query: dict):
...
def aoj_apply_action_agg_job():
    instances = aoj_apply_action_agg_get_sqlinstance()
    instances.map(aoj_apply_action_agg_query_on_db).collect()
