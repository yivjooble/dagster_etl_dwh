...
PROCEDURE_CALL = "call an.prc_vacancy_collars(%s);"
...
CUSTOM_COUNTRY_LIST = [
    'at', 'uk', 'de', 'fr', 'us', 'ca', 'ch', 'ro', 'pl', 'nl', 'cz', 'be', 'au'
]


@op(out=DynamicOut(), required_resource_keys={'globals'})
def vacancy_collars_get_sqlinstance(context):
    '''
...
def vacancy_collars_query_on_db(context, sql_instance_country_query: dict):
...
@job(
    ...
    metadata={
        "input_date": f"{TODAY_DATE} - {TODAY_DATE}",
...
def vacancy_collars_job():
    instances = vacancy_collars_get_sqlinstance()
    instances.map(vacancy_collars_query_on_db).collect()
