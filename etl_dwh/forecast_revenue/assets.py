...
TABLE_NAME = 'revenue_month_forecast_new'
SCHEMA = 'ono'
CLOUDBERRY_TABLE_NAME = 'revenue_month_forecast'
...
@asset(group_name='revenue_month_forecast_new_assets',
       key_prefix='revenue_month_forecast_new_assets',
       compute_kind='sql')
...
@asset(group_name='revenue_month_forecast_new_assets',
       key_prefix='revenue_month_forecast_new_assets')
def get_unique_channels(load_data):
    """
...
@asset(group_name='revenue_month_forecast_new_assets',
       key_prefix='revenue_month_forecast_new_assets')
def prepare_forecasts(get_unique_channels, load_data) -> pd.DataFrame:
    """
...
@asset(group_name='revenue_month_forecast_new_assets',
       key_prefix='revenue_month_forecast_new_assets')
def save_forecasts_to_db(context, prepare_forecasts):
    """
    Save the consolidated forecasts to db.
    """
    logger = get_dagster_logger()
    res_df = prepare_forecasts
