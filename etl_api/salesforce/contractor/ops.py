...
FILE_NAME = "sf_contractor"
SCHEMA = "salesforce"
OBJECT_NAME = "Contractor__c"
...
FIELD_MAPPING = {
    'id_contractor': 'Id1C__c',           # External ID field
    'contractor_size': 'ClientSegment__c',
    'mtd_revenue_usd': 'MonthlyRevenueUSD__c'
}

class Contractor(BaseSalesforceOps):
    def __init__(self, domain, source_db):
        super().__init__(domain=domain)
        self.field_mapping = FIELD_MAPPING
        self.source_db = source_db

    def update_contractor(self, context):
        """
        ...
        try:
            ...
            context.log.info(f"Retrieved {len(df)} records to update")

            # Convert back to records for Salesforce upload
            ...
            job_result = self.upload_data_csv(
                ...
                operation='upsert',
                external_id_field=self.field_mapping['id_contractor'],
                context=context
            )
            ...
            context.log.error(f"Error updating contractor sizes: {str(e)}")
            raise


@op(required_resource_keys={'globals'})
def update_contractor(context):
    ops = Contractor(
        domain=context.resources.globals['domain'],
        source_db=context.resources.globals['source_db']
    )
    ops.update_contractor(context)
...
@job(
    ...
    description="""Updates Account contractor sizes and related fields in Salesforce.
                   Runs monthly on the 1st day of the month.
                   Passes last day of previous month as a parameter."""
)
def sf_contractor_job():
    update_contractor()
