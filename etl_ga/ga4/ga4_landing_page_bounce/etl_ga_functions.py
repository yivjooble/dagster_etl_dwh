...
def report_to_dataframe(response, table_name: str):
    ...
    for report in response.get('reports', []):
        ...
        mapped_dimensions = [mapped for i, input_fields in enumerate(
            request_params[table_name]['dimensions']) for i, mapped in map_column_names.items() if i == input_fields['name']]
        mapped_metrics = [mapped for i, input_fields in enumerate(
            request_params[table_name]['metrics']) for i, mapped in map_column_names.items() if i == input_fields['name']]
...
def delete_from_dwh_db(context, schema, table_name, date_start, date_end, delete_countries):
    try:
        with dwh_conn_psycopg2() as con:
            with con.cursor() as cur:
                cur.execute(
                    f'''
                    delete from {schema}.{table_name}
                    where action_date between '{date_start}' and '{date_end}'
                        and country_id in ({delete_countries})
                    '''
                )

                context.log.info(f"delete from {schema}.{table_name}\n"
                                f"where action_date between '{date_start}' and '{date_end}\n"
                                f"and country_id in ({delete_countries})"
                                )
    except Exception:
        raise Exception
