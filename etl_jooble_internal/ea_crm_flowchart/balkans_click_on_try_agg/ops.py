...
TABLE_NAME = "balkans_click_on_try_agg"
SCHEMA = "balkans"
...
@op(out=DynamicOut())
def balkans_click_on_try_agg_get_sqlinstance(context):
    '''
    ...
    for sql_instance in Operations.generate_sql_instance(
            ...
            db_name="DteFlowchart",
            query=GITLAB_SQL_Q):
        yield sql_instance


@op(retry_policy=retry_policy, required_resource_keys={'globals'})
def balkans_click_on_try_agg_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def balkans_click_on_try_agg_job():
    instances = balkans_click_on_try_agg_get_sqlinstance()
    instances.map(balkans_click_on_try_agg_query_on_db)
