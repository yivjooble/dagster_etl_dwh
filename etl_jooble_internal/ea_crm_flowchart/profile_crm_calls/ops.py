...
@op(out=DynamicOut())
def profile_crm_calls_get_sqlinstance(context):
    '''
...
def profile_crm_calls_query_on_db(context, sql_instance_country_query: dict):
    '''
    ...
    try:
        ...
        if not context.resources.globals["max_id"]:
            query = "SELECT MAX(id) as max_id FROM dte.profile_crm_calls"
...
def profile_crm_calls_job():
    instances = profile_crm_calls_get_sqlinstance()
    instances.map(profile_crm_calls_query_on_db)
