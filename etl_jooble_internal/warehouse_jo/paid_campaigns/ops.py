...
@op(out=DynamicOut())
def paid_campaigns_get_sqlinstance(context):
    '''
...
def paid_campaigns_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def paid_campaigns_job():
    instances = paid_campaigns_get_sqlinstance()
    instances.map(paid_campaigns_query_on_db)
