...
@op(out=DynamicOut())
def paid_info_currency_get_sqlinstance(context):
    '''
...
def paid_info_currency_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def paid_info_currency_job():
    instances = paid_info_currency_get_sqlinstance()
    instances.map(paid_info_currency_query_on_db)
