...
@op(out=DynamicOut())
def paid_accounts_get_sqlinstance(context):
    '''
...
def paid_accounts_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def paid_accounts_job():
    instances = paid_accounts_get_sqlinstance()
    instances.map(paid_accounts_query_on_db)
