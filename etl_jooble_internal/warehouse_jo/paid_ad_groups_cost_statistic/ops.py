...
TABLE_NAME = "paid_ad_groups_cost_statistic"
SCHEMA = "traffic"
DATE_COLUMN = 'date'
DATE_START = (datetime.now().date() - timedelta(3)).strftime('%Y-%m-%d')
...
def paid_ad_groups_cost_statistic_get_sqlinstance(context):
    '''
...
def paid_ad_groups_cost_statistic_query_on_db(context, sql_instance_country_query: dict):
    '''
...
@job(
    config=job_config,
    resource_defs={
        "globals": make_values_resource(reload_date_start=Field(str, default_value=DATE_START),
...
def paid_ad_groups_cost_statistic_job():
    instances = paid_ad_groups_cost_statistic_get_sqlinstance()
    instances.map(paid_ad_groups_cost_statistic_query_on_db)
