...
TABLE_NAME = "balkans_competitor_scans_infostud"
SCHEMA = "balkans"
...
@op(out=DynamicOut())
def balkans_competitor_scans_infostud_get_sqlinstance(context):
    '''
    ...
    for sql_instance in Operations.generate_sql_instance(
            ...
            instance_name="amt_projects",
            db_name="AMT_projects",
            query=GITLAB_SQL_Q):
        yield sql_instance


@op(required_resource_keys={"globals"}, retry_policy=retry_policy)
def balkans_competitor_scans_infostud_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def balkans_competitor_scans_infostud_job():
    instances = balkans_competitor_scans_infostud_get_sqlinstance()
    instances.map(balkans_competitor_scans_infostud_query_on_db)
