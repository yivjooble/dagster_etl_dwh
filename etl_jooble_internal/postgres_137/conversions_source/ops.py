...
@op(out=DynamicOut())
def conversions_source_get_sqlinstance(context):
    '''
...
def conversions_source_query_on_db(context, sql_instance_country_query: dict):
    '''
    ...
    try:
        ...
        if not context.resources.globals["max_id"]:
            query = "SELECT MAX(id) as max_id FROM imp_statistic.conversions_source"

            max_id = DwhOperations.execute_on_dwh(
                context=context, query=query, fetch_results=True)
        ...
        DwhOperations.execute_on_dwh(context, deletion_query, destination_db=destination_db)
...
def conversions_source_job():
    instances = conversions_source_get_sqlinstance()
    instances.map(conversions_source_query_on_db)
