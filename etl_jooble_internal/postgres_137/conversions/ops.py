...
@op(out=DynamicOut())
def conversions_get_sqlinstance(context):
    '''
...
def conversions_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def conversions_job():
    instances = conversions_get_sqlinstance()
    instances.map(conversions_query_on_db)
