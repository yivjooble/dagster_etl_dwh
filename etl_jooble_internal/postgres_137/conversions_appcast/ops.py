...
TABLE_NAME = "conversions_appcast"
...
@op(out=DynamicOut())
def conversions_appcast_get_sqlinstance(context):
    """
...
def conversions_appcast_launch_query_on_db(context, sql_instance_country_query: dict):
    """
    ...
    if not context.resources.globals["max_id"]:
        query = "SELECT MAX(id) as max_id FROM imp_statistic.conversions_appcast"
    ...
    deletion_query = f"""DELETE
                         FROM imp_statistic.conversions_appcast
...
def conversions_appcast_job():
    db_instances = conversions_appcast_get_sqlinstance()
    db_instances.map(conversions_appcast_launch_query_on_db)
