...
@op(out=DynamicOut())
def undelivered_email_get_sqlinstance(context):
    '''
...
def undelivered_email_query_on_db(context, sql_instance_country_query: dict):
    '''
    ...
    try:
        ...
        sql_instance_country_query["to_sql_reload_date"] = reload_date
        ...
        # delete previously stored data from dwh
        DwhOperations.delete_data_from_dwh_table(
            context=context,
            schema=SCHEMA,
            table_name=TABLE_NAME,
            date_column=DATE_COLUMN,
            date_start=reload_date,
...
@job(config=job_config,
     resource_defs={"globals": make_values_resource(reload_date=Field(str, default_value=YESTERDAY_DATE),
...
def undelivered_email_job():
    instances = undelivered_email_get_sqlinstance()
    instances.map(undelivered_email_query_on_db)
