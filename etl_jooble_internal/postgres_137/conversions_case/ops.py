...
@op(out=DynamicOut())
def conversions_case_get_sqlinstance(context):
    '''
...
def conversions_case_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def conversions_case_job():
    instances = conversions_case_get_sqlinstance()
    instances.map(conversions_case_query_on_db)
