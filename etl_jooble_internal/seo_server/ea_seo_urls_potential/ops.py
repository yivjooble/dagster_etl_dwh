...
TABLE_NAME = "ea_seo_urls_potential"
SCHEMA = "traffic"
PREVIOUS_MONTH_START = (datetime.now().replace(day=1) - timedelta(days=1)).replace(day=1).strftime('%Y-%m-%d')
PREVIOUS_MONTH_END = (datetime.now().replace(day=1) - timedelta(days=1)).strftime('%Y-%m-%d')
...
def ea_seo_urls_potential_get_sqlinstance(context):
    """
...
def ea_seo_urls_potential_query_on_db(context, sql_instance_country_query: dict):
    """
    ...
    """
    previous_month_start = context.resources.globals["reload_date_start"]
    previous_month_end = context.resources.globals["reload_date_end"]
    country_code = sql_instance_country_query["country_code"]

    sql_instance_country_query['to_sql_previous_month_start'] = previous_month_start
    sql_instance_country_query['to_sql_previous_month_end'] = previous_month_end
    sql_instance_country_query['to_sql_country_code'] = country_code
...
@job(
    ...
    resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=CUSTOM_COUNTRIES),
                                                   reload_date_start=Field(str, default_value=PREVIOUS_MONTH_START),
                                                   reload_date_end=Field(str, default_value=PREVIOUS_MONTH_END),
                                                   ),
                   "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"})},
    name=generate_job_name(TABLE_NAME),
    tags={"data_model": f"{SCHEMA}"},
    metadata={
        "input_date": f"{PREVIOUS_MONTH_START} - {PREVIOUS_MONTH_END}",
...
def ea_seo_urls_potential_job():
    db_instances = ea_seo_urls_potential_get_sqlinstance()
    db_instances.map(ea_seo_urls_potential_query_on_db).collect()
