...
@op(out=DynamicOut())
def seo_test_groups_get_sqlinstance(context):
    '''
...
def seo_test_groups_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def seo_test_groups_job():
    instances = seo_test_groups_get_sqlinstance()
    instances.map(seo_test_groups_query_on_db)
