...
TABLE_NAME = "crawler_stat"
SCHEMA = "traffic"
COUNTRY_COLUMN = "country_code"
DATE_COLUMN = "date"
START_DATE = ((datetime.now() - timedelta(days=3)).date().isoformat())
END_DATE = ((datetime.now() - timedelta(days=2)).date().isoformat())  # 2 days gap on source data
GITLAB_SQL_Q, GITLAB_SQL_URL = fetch_gitlab_data(
    config_key="default",
    dir_name=SCHEMA,
    file_name=TABLE_NAME,
)
CUSTOM_COUNTRY_LIST = ["us", "uk", "de", "fr", "pl"]


@op(required_resource_keys={"globals"}, out=DynamicOut())
def crawler_stat_get_sqlinstance(context):
    """
...
def crawler_stat_query_on_db(context, sql_instance_country_query: dict):
    """
    ...
    Returns:
        list: The file path of the executed query.
    """
    ...
    to_sqlcode_tbl_crawler_stat = f"{country_code}_crawler_stat"
    to_sqlcode_tbl_region = f"{country_code}_region"
    ...
    formatted_query = sql_instance_country_query["query"].format(
        ...
        to_sqlcode_tbl_crawler_stat=to_sqlcode_tbl_crawler_stat,
        to_sqlcode_tbl_region=to_sqlcode_tbl_region,
...
@job(
    ...
    resource_defs={
        "globals": make_values_resource(reload_countries=Field(list, default_value=CUSTOM_COUNTRY_LIST),
                                        reload_date_start=Field(str, default_value=START_DATE),
    ...
    metadata={
        "input_date": f"{START_DATE} - {END_DATE}",
...
def crawler_stat_job():
    db_instances = crawler_stat_get_sqlinstance()
    db_instances.map(crawler_stat_query_on_db)
