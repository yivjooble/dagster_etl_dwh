...
@op(out=DynamicOut())
def campaign_cpa_get_sqlinstance(context):
    '''
...
def campaign_cpa_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def campaign_cpa_job():
    instances = campaign_cpa_get_sqlinstance()
    instances.map(campaign_cpa_query_on_db)
