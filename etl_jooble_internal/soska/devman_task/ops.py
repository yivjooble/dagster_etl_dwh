...
@op(out=DynamicOut())
def devman_task_get_sqlinstance(context):
    '''
...
def devman_task_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def devman_task_job():
    instances = devman_task_get_sqlinstance()
    instances.map(devman_task_query_on_db)
