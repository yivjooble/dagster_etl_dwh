...
@op(out=DynamicOut())
def balkans_payments_get_sqlinstance(context):
    """
...
def balkans_payments_query_on_db(context, sql_instance_country_query: dict):
    """
...
@job(
    config=job_config,
    resource_defs={"globals": make_values_resource(destination_db=Field(str, default_value="cloudberry")),
...
def balkans_payments_job():
    db_instances = balkans_payments_get_sqlinstance()
    db_instances.map(balkans_payments_query_on_db)
