...
TABLE_NAME = "subscription_model_jobs"
SCHEMA = "aggregation"
...
def subscription_model_jobs_get_sqlinstance(context):
    '''
    ...
    for sql_instance in Operations.generate_sql_instance(
            ...
            db_name="employer_statistics",
            query=GITLAB_SQL_Q):
        yield sql_instance


@op(required_resource_keys={"globals"}, retry_policy=retry_policy)
def subscription_model_jobs_query_on_db(context, sql_instance_country_query: dict):
    '''
    ...
    sql_instance_country_query['to_sqlcode_date_start'] = operation_date_diff_start
    sql_instance_country_query['to_sqlcode_date_end'] = operation_date_diff_end
...
def subscription_model_jobs_job():
    instances = subscription_model_jobs_get_sqlinstance()
    instances.map(subscription_model_jobs_query_on_db)
