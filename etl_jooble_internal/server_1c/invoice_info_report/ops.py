...
TABLE_NAME = "invoice_info_report"
...
@op(out=DynamicOut())
def invoice_info_report_get_sqlinstance(context):
    '''
    ...
    for sql_instance in Operations.generate_sql_instance(
            ...
            instance_name="1c",
            db_name="invoiceReport",
            query=GITLAB_SQL_Q):
        yield sql_instance


@op(retry_policy=retry_policy, required_resource_keys={'globals'})
def invoice_info_report_query_on_db(context, sql_instance_country_query: dict):
    '''
...
def invoice_info_report_job():
    instances = invoice_info_report_get_sqlinstance()
    instances.map(invoice_info_report_query_on_db)
