...
RPL_DB = 'us'
RPL_SCHEMA = 'an'
RPL_TABLE_NAME = 'job_cluster_historical_data'
RPL_PROCEDURE = 'call an.prc_job_cluster_historical_data();'
CH_SCHEMA = 'us'
CH_TABLE_NAME = "historical_job_cluster_transfer"
...
def historical_job_cluster_transfer_save_data_as_parquet(context):
    """
    Save data.
    """
...
@op(out=DynamicOut())
def historical_job_cluster_transfer_read_parquet(context, parquets):
...
@op
def historical_job_cluster_transfer_save_parquet_to_ch(context, params: dict):
...
@op
def historical_job_cluster_transfer_drop_useless(context, save_parquet):
    delete_parquet_files(context, PATH_TO_DATA)

    # Drop replica table
    execute_on_rpl(
        sql="drop table if exists an.job_cluster_historical_data;",
        host=RPL_HOST,
        dbname=RPL_DB
    )
...
@job(config=job_config,
     resource_defs={"globals": make_values_resource(reload_countries=Field(list, default_value=CUSTOM_COUNTRIES_LIST),
                                                    ...
                                                    npartitions=Field(int, default_value=200)),
                    "io_manager": fs_io_manager.configured({"base_dir": f"{get_io_manager_path()}"})},
     name=JOB_PREFIX + CH_TABLE_NAME,
     description='Copy data from postgres_replica to clickhouse db.')
def historical_job_cluster_transfer_job():
    """
    Description: Load historical job_cluster table from replica database to Clickhouse
    Connects to: replica database, Clickhouse
    """
    # get data from history database
    to_parquet = historical_job_cluster_transfer_save_data_as_parquet()
    # save data from history database as parquet
    read_parquet = historical_job_cluster_transfer_read_parquet(to_parquet)
    save_parquet = read_parquet.map(historical_job_cluster_transfer_save_parquet_to_ch).collect()
    historical_job_cluster_transfer_drop_useless(save_parquet)
