...
# Define setting
STORAGE_TABLE_NAME = "info_currency"  # name of table in Clickhouse
...
def info_currency_create_dict_params(context):
    """
...
def info_currency_save_data_as_pkl(context, dict_params: dict) -> str:
    """
...
@op(ins={"file_paths": In(List[str])},
    required_resource_keys={'globals'})
def info_currency_ch_truncate_table(context, **kwargs):
    """
    Truncate table.
    """
...
def info_currency_ch_insert_date(context, delete, file_paths):
    """
    Insert data.
    """
...
def info_currency_job():
    """
    Description: Load info_currency table from replica database to Clickhouse
    Connections: replica database, Clickhouse
    """
    # get data from history database
    create_dict_params = info_currency_create_dict_params()

    # save data from history database as pkl to /data
    pkl_files_path = create_dict_params.map(info_currency_save_data_as_pkl).collect()

    # delete data from Clickhouse table
    ch_delete = info_currency_ch_truncate_table(pkl_files_path)

    # insert data to Clickhouse table
    info_currency_ch_insert_date(ch_delete, pkl_files_path)
