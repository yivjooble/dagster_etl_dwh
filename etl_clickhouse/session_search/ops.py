...
# Define setting
STORAGE_TABLE_NAME = "session_search"  # name of table in Clickhouse
...
def session_search_create_dict_params(context):
    """
...
def session_search_save_data_as_pkl(context, dict_params: dict):
    """
...
def session_search_job():
    """
    Description: Load session_search table from replica database to Clickhouse
    ...
    create_dict_params = session_search_create_dict_params()

    # save data from history database as pkl to /data
    create_dict_params.map(session_search_save_data_as_pkl).collect()
