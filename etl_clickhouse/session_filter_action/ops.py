...
# Define setting
STORAGE_TABLE_NAME = "session_filter_action"  # name of table in Clickhouse
...
def session_filter_action_create_dict_params(context):
    """
    ...
    for cluster in clusters.values():
        for country in cluster['dbs']:
            ...
            for launch_country in launch_countries:
                if str(country).lower() in str(launch_country).lower():
                    ...
                    yield DynamicOutput(
                        ...
                        mapping_key='history_' + country
                    )


@op(required_resource_keys={'globals'},
    retry_policy=retry_policy)
def session_filter_action_save_data_as_pkl(context, dict_params: dict):
    """
            ...
            if file_path.endswith(".pkl"):
                ...
                with gzip.open(file_path, 'rb') as f:
                    ...
                    if not df.empty:
                        ...
                        context.log.info(
                            f'Data deleted for: {STORAGE_TABLE_NAME}, {country_name.upper()}, {date}, {date}')
...
def session_filter_action_job():
    """
    Description: Load session_filter_action table from replica database to Clickhouse
    ...
    create_dict_params = session_filter_action_create_dict_params()

    # save data from history database as pkl to /data
    create_dict_params.map(session_filter_action_save_data_as_pkl).collect()
