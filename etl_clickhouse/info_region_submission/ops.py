...
# Define setting
STORAGE_TABLE_NAME = "info_region_submission"  # name of table in Clickhouse
...
def info_region_submission_create_dict_params(context):
    """
...
def info_region_submission_save_data_as_pkl(context, dict_params: dict) -> str:
    """
...
@op(ins={"file_paths": In(List[str])},
    required_resource_keys={'globals'})
def info_region_submission_ch_truncate_table(context, **kwargs):
    """
    Truncate table.
    """
...
def info_region_submission_ch_insert_date(context, delete, file_paths):
    """
    Insert data.
    """
...
def info_region_submission_job():
    """
    Description: Load info_region_submission table from replica database to Clickhouse
    Connections: replica database, Clickhouse
    Settings: TABLE_NAME, DELETE_DATE_COLUMN, PATH_TO_DATA
    """
    # get data from history database
    create_dict_params = info_region_submission_create_dict_params()

    # save data from history database as pkl to /data
    pkl_files_path = create_dict_params.map(info_region_submission_save_data_as_pkl).collect()

    # delete data from Clickhouse table
    ch_delete = info_region_submission_ch_truncate_table(pkl_files_path)

    # insert data to Clickhouse table
    info_region_submission_ch_insert_date(ch_delete, pkl_files_path)
