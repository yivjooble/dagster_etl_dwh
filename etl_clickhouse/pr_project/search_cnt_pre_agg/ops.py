...
from etl_clickhouse.pr_project.search_cnt_pre_agg.sql.search_cnt_pre_agg import generate_aggregation_query_to_run
...
# Define setting
STORAGE_TABLE_NAME = "search_cnt_pre_agg"  # name of table in Clickhouse
DELETE_DATE_COLUMN = "date_diff"
...
def search_cnt_pre_agg_create_dict_params(context):
    """
...
@op(retry_policy=retry_policy,
    required_resource_keys={'globals'})
def search_cnt_pre_agg_save_data_as_pkl(context, dict_params: dict):
    """
    ...
    for raw_date in date_range:
        ...
        try:
            ...
            context.log.info(f'Data deleted from: {country_name}.{STORAGE_TABLE_NAME}, {date_diff}, {date_diff}')
...
def search_cnt_pre_agg_job():
    """
    Description: Aggregate raw data for pr-project.
    Connects to: Clickhouse
    """
    create_dict_params = search_cnt_pre_agg_create_dict_params()
    create_dict_params.map(search_cnt_pre_agg_save_data_as_pkl)
